...
public class QueueServerConnection {
    ...
    /**
     * Sends a request to the server to insert a DVD into the queue.
     *
     * @param dvdToSend the DVD to be inserted into the queue
     * @return the status of the operation (true == successful)
     * @throws IOException thrown if there is a problem with the connection
     */
    public boolean writeDVD(DVD dvdToSend) throws IOException {
        Socket clientSocket = new Socket(host, port);
        ObjectOutputStream outToServer = new ObjectOutputStream(clientSocket.getOutputStream());
        ObjectInputStream inFromServer = new ObjectInputStream(clientSocket.getInputStream());
        outToServer.writeObject(dvdToSend);
