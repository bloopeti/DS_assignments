 ...
 * @Since: Sep 1, 2015
 * @Description: Serves for the connection of the client with the queue server.
...
public class QueueServerConnection {
    ...
    /**
     * Retrieves a dvd from the queue of the server, by sending a dvd with the title "GET_NEW_DVD" to it.
     * If the queue is empty, this method will hang until the queue will get a message. In
     * other words, this method will wait for the server to provide a message.
     *
     * @return the dvd from the queue, sent by the server
     * @throws IOException thrown if there is a problem with the connection
     */
    public DVD readDVD() throws IOException {
        Socket clientSocket = new Socket(host, port);
        ObjectOutputStream outToServer = new ObjectOutputStream(clientSocket.getOutputStream());
        ObjectInputStream inFromServer = new ObjectInputStream(clientSocket.getInputStream());
        outToServer.writeObject(new DVD("GET_NEW_DVD", -1, -1));

        DVD response;
        try {
            response = (DVD) inFromServer.readObject();
        } catch (ClassNotFoundException e) {
            response = null;
            e.printStackTrace();
        }
        ...
        if (response == null || !response.getClass().equals(DVD.class)) return null;
