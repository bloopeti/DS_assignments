...
public class CatalogController {
    ...
    private class DeleteActionListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            try {
                int studentId = Integer.parseInt(catalogView.getDeleteId());
                //encode the request: a POST request, with url "student?id=X" (passing id in url) and no object sent through
                String encodedRequest = RequestMessageEncoder.encode(ProtocolMethod.GET, "student?deleteid=" + studentId);
                String response = serverConnection.sendRequest(encodedRequest);
                ResponseMessage decodedResponse = ResponseMessageEncoder.decode(response);
